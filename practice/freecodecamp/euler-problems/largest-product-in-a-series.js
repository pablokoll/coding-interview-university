...
function largestProductinaSeries(n) {
    ...
    for (let i = 0; i < thousandDigits.length; i++) {
        const combination = thousandDigits.slice(i, i + n);
        if (combination.includes(0) || combination.includes(undefined)) {
            continue;
        }
        const result = combination.reduce((a, b) => a * b, 1)
        console.log(combination, ' result ', result)
        if (result > maxCombination) {
